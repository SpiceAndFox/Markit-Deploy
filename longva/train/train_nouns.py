...
class DataArguments:
    ...
    input_format: str = field(default="Answer in the format: 'From Frame x to Frame y'.")
...
def make_instruction_with_colors(nouns_dict: dict) -> str:
    """
    video_info 里有:
    - "nouns": {"1": "person", "2": "broom", "3": "floor", ...}
    """
    
    base_instruction = "The black numbers on each frame represent the frame number."
...
def preprocess_qwen(
    ...
    tokenizer: transformers.PreTrainedTokenizer,
    has_image: bool = False,
    nouns_dict: Optional[dict] = None,   # ✅ 新增
    input_format: str = "Answer in the format: 'From Frame x to Frame y'.",   # ✅ 新增
) -> Dict:
    conv = conversation_lib.default_conversation.copy()
    roles = {"human": conv.roles[0], "gpt": conv.roles[1]}

    # ✅ 构造 full_instruction（nouns_dict 为空时也会返回 base_instruction）
    full_instruction = make_instruction_with_colors(nouns_dict)
    ...
    for i, source in enumerate(sources):
        ...
        # ============================================================
        # ✅ 1) 确认一个 source 只有一个 human query（单轮）
        # ============================================================
        human_idxs = [k for k, s in enumerate(source) if s.get("from") == "human"]
        if len(human_idxs) != 1:
            raise ValueError(
                f"Expect exactly 1 human query per source, but got {len(human_idxs)} "
                f"at sample index {i}. (conversation id unknown here)"
            )

        h_idx = human_idxs[0]

        # ============================================================
        # ✅ 2) 插入 full_instruction 到 human prompt，不影响其余逻辑
        #    - 优先插在 <image>\n 后
        #    - 如果没有 <image>，则补一个
        # ============================================================
        hv = source[h_idx].get("value", "")

        # 仅插入，不补 <image>
        if hv.startswith("<image>\n"):
            hv = "<image>\n" + full_instruction + "\n" + hv[len("<image>\n"):]
        elif hv.startswith("<image>"):
            hv = "<image>\n" + full_instruction + "\n" + hv[len("<image>"):].lstrip("\n")
        else:
            hv = full_instruction + "\n" + hv   # ✅ 不补 <image>

        source[h_idx]["value"] = hv
        ...
        #     prefix = "<image>\n"   # 统一成有换行的形式
        #     rest = rest[len("<image>"):].lstrip("\n")

        # rest = rest.rstrip()

        # # 2) 防止重复追加（跑多次或数据里已含）
        # if suffix not in rest:
        #     if len(rest) > 0 and not rest.endswith((".", "?", "!", "\"", "'")):
        #         rest += "."
        #     rest = rest + "\n" + suffix

        # source[h_idx]["value"] = prefix + rest
    ...
    # Tokenize conversations
    # print("concersation", conversations)
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version == "qwen_1_5":
        return preprocess_qwen(sources, tokenizer, has_image=has_image,nouns_dict=nouns_dict)
    # add end signal and concatenate together
    conversations = []
    for source in sources:
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            if 'image' in sources[0]:
                ...
                video_file = f"{self.list_data_dict[i]['id']}.mp4"
                ...
                if not os.path.isfile(video_path): 
                    print(f"⚠️ Missing video, skip idx={i}: {video_path}")
            ...
            data_dict = preprocess_qwen(
                ...
                nouns_dict=self.list_data_dict[i].get("nouns") or None,
                input_format=self.data_args.input_format,            
                )
...
class SaveLoraCallback(transformers.TrainerCallback):
    def on_save(self, args, state, control, **kwargs):
        ...
        ckpt_dir = os.path.join(args.output_dir, f"lora-step-{state.global_step}")
        os.makedirs(ckpt_dir, exist_ok=True)

        # 导出 LoRA state_dict（兼容 deepspeed zero3）
