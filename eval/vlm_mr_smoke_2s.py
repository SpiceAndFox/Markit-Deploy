...
DEFAULT_NUMPRO_INSTRUCTION = "The red numbers on each frame represent the frame number."
...
def annotate_and_save_video(
    ...
    while True:
        ...
        if frame_count % sample_interval == 0:
            ...
            if draw_text:
                frame_small = annotate_frame_with_pil(
                    frame_small,
                    str(frame_number),
                    position,
                    font_size,
...
def process_dataset(args):
    ...
    temp_dir = tempfile.mkdtemp(prefix=f"{args.backend}_{args.protocol}_2s_", dir=args.temp_dir_root)
    try:
        for record in tqdm(records):
            ...
            annotate_and_save_video(
                ...
                color=args.color,
                draw_text=(args.protocol == "numpro"),
            )
