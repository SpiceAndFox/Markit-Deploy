...
IMPLEMENTATION_VERSION = "direct_overlay_raw_noannot_v1"
...
def probe_video_fps(video_path: str) -> float:
    fps, _ = probe_video_metadata(video_path)
    return fps


def try_probe_video_fps(video_path: str) -> float | None:
    try:
        return probe_video_fps(video_path)
    except RuntimeError:
        return None
...
def process_dataset(args):
    ...
    for record in tqdm(records):
        ...
        try:
            ...
            fallback_video_path = resolve_raw_video_path(args.raw_video_root, record)
            if fallback_video_path == video_path:
                raise

            render_mode = "raw_direct"
            video_path = fallback_video_path
            instruction = normalize_text(args.number_instruction)
            prompt = compose_prompt(instruction, args.input_format.format(record["query"]))
            tqdm.write(
                f"[fallback] {record['id']} overlay decode failed, retrying raw video: {exc}"
            )
            source_fps, source_total_frames = probe_video_metadata(video_path)
            sampling_metadata = build_sampling_metadata(
                backend_name=args.backend,
