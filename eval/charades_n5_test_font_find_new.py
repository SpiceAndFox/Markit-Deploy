...
def video_demo(
    ...
    if not os.path.exists(visual_path):
        ...
        visual_path = os.path.join(raw_video_root, video_info['video'])
        render_mode = "markit_fallback"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mask_path", type=str, default="", help="Optional directory containing MarkIt masks named <id>_mask.npy.")
    parser.add_argument("--raw_video_root", type=str, default="", help="Optional fallback raw-video directory.")
    parser.add_argument("--save_path", type=str, required=True, help="Path to write JSON predictions.")
    parser.add_argument("--test_path", type=str, required=True, help="Charades/ActivityNet-style test annotation JSON.")
    ...
    video_list = load_test_records(testset_path)
    ...
    for i, video_info in enumerate(tqdm(video_list), 1):
        ...
        response = video_demo(model, video_info, data_path, args.raw_video_root, num_sampled_frames, input_format, instruction, position, font_size, color, mask_path=args.mask_path)
